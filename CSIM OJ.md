...
13. 隊伍api (team)
    ...
    | GET        | URL/team/correctedInfo**        | 取得此學生**被**批改的資訊   | problemId                                                    | [{studentAccount, score(分數), correctValue(程式正確性), readValue(程式可讀性), skillValue(技巧運用), completeValue(程式完整性), wholeValue(綜合評分)}] |
